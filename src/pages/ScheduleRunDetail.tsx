...
export default function ScheduleRunDetail() {
  ...
  if (!run) {
    return (
      <>
        ...
        <div className="empty">{t('sch.run.detail.missing')}</div>
      </>
    );
  }
  ...
  const task = db.tasks.find((candidate) => candidate.id === run.task_id);
  const taskRun = run.task_run_id
    ? db.taskRuns.find((candidate) => candidate.id === run.task_run_id)
    : undefined;
  ...
  return (
    <>
      <div className="toolbar detail-toolbar">
        <div className="left">
          <BackLink to={schedule ? `/schedules/${schedule.id}` : '/schedules'} label={t('sch.run.detail.back')} />
        </div>
        <div className="left">
          {run.task_run_id && (
            <button className="btn sm" type="button" onClick={() => navigate(`/task-runs/${run.task_run_id}`)}>
              {t('sch.col.taskRun')} {taskRun?.id ?? run.task_run_id}
            </button>
          )}
        </div>
      </div>
      ...
      <DetailHero
        ...
        side={
          <>
            ...
            <span className="chip violet">{run.trigger_type}</span>
            {run.reason && <span className="chip amber">{run.reason}</span>}
          </>
        }
      />
      ...
      <div className="stat-grid detail-stats">
        <div className="stat stat-wide">
          <div className="stat-wide-head">
            <div>
              <div className="stat-val sm">{timeShort(run.triggered_at)}</div>
              <div className="stat-label">{t('sch.run.detail.triggered')}</div>
            </div>
            <span className="chip violet">{run.trigger_type}</span>
          </div>
        </div>
        <div className="stat">
          <div className="stat-val sm"><StatusBadge status={run.status} /></div>
          <div className="stat-label">{t('dash.col.status')}</div>
        </div>
        <div className="stat">
          <div className="stat-val sm">{run.jitter_applied_seconds}s</div>
          <div className="stat-label">{t('sch.col.jitter')}</div>
        </div>
        {run.task_run_id && (
          <div className="stat">
            <div className="stat-val sm green">{run.task_run_id}</div>
            <div className="stat-label">{t('sch.col.taskRun')}</div>
          </div>
        )}
      </div>
      ...
      <div className="two-col detail-grid">
        <Panel title={t('sch.run.detail.relationships')}>
          <dl className="kv detail-kv">
            ...
            <dd>
              <button className="btn ghost sm relationship-link" type="button" onClick={() => navigate(`/schedules/${run.schedule_id}`)}>
                {schedule?.name ?? run.schedule_id}
              </button>
            </dd>
            <dt>{t('sch.col.task')}</dt>
            <dd>
              <button className="btn ghost sm relationship-link" type="button" onClick={() => navigate(`/tasks/${run.task_id}`)}>
                {task?.name ?? run.task_id}
              </button>
            </dd>
            <dt>{t('dash.col.jobDefinition')}</dt>
            <dd>
              <button className="btn ghost sm relationship-link" type="button" onClick={() => navigate(`/job-definitions/${run.bot_id}`)}>
                {bot?.code ?? run.bot_id}
              </button>
            </dd>
            {run.task_run_id && (
              <>
                <dt>{t('sch.col.taskRun')}</dt>
                <dd className="mono">
                  <button className="btn ghost sm relationship-link" type="button" onClick={() => navigate(`/task-runs/${run.task_run_id}`)}>
                    {taskRun?.id ?? run.task_run_id}
                  </button>
                </dd>
              </>
            )}
        ...
        <Panel title={t('sch.run.detail.result')}>
          <dl className="kv detail-kv">
            ...
            <dt>{t('sch.run.detail.taskOutcome')}</dt>
            <dd>
              {run.task_run_id
                ? <span className="chip green">{t('sch.run.detail.taskCreated')}</span>
                : <span className="chip amber">{t('sch.run.detail.noTask')}</span>}
            </dd>
